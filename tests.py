...
from schemas.schemas import MovementType
...
def test_get_client():
    id = pytest.client["id"]
    response = clientTest.get(f"/clients/{id}")
...
def test_create_account_erasedClient():
    account_data = {
        "client_id": pytest.client["id"],
...
def test_update_account():
    updated_account_data = {"id":pytest.account['id'],"name": "Updated Account", "balance": 2000.0}
...
def test_create_category_alreadyCreated():
    category_data = {
...
def test_remove_category_from_client():
    response = clientTest.delete(f"/clients/{pytest.clientcategory['client_id']}/categories/{pytest.clientcategory['category_id']}")
    assert response.status_code == 204
...
""" reset db
def deletealltest():
    response = clientTest.delete(f"/all")
    assert response.status_code == 200
def delete_all():
    deletealltest()
delete_all()
